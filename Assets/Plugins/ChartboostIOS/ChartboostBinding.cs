...
namespace Prime31
{
	public class ChartboostBinding
	{
		...
		[DllImport("__Internal")]
		private static extern void _chartboostShowNewsFeedUI();
	
		// Shows the news feed UI
		public static void showNewsFeedUI()
		{
			if( Application.platform == RuntimePlatform.IPhonePlayer )
				_chartboostShowNewsFeedUI();
		}
	
	
		[DllImport("__Internal")]
		private static extern void _chartboostCloseNewsFeedUI();
	
		// Closes the news feed UI
		public static void closeNewsFeedUI()
		{
			if( Application.platform == RuntimePlatform.IPhonePlayer )
				_chartboostCloseNewsFeedUI();
		}
	}
