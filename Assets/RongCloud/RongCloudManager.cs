...
namespace RongCloud
{
    public class RongCloudManager : MonoBehaviour
    {
        ...
        public static event Action<long> onSendMessageSuccessEvent;
        public static event Action<RCMessage> onSentEvent;
        public static event Action<long,RCErrorCode> onSendMessageFailedEvent;

        public void onSendMessageSuccess (string messageIdStr)
        {
            Debug.Log ("onSendMessageSuccess : " + messageIdStr);
        ...
        public void onSendMessageFailed (string json)
        {
            ...
            if (onSendMessageFailedEvent != null) {
                onSendMessageFailedEvent (messageId, errorCode);
            }
        }
        ...
        //发送消息的回调，不管消息有没有成功
        public static event Action<RCMessage> onSendMessageResultEvent;

        public void onSendMessageResult (string json)
        {
            Debug.Log ("onSendMessageResult : " + json);
            RCMessage message = RCMessage.DecodeFromJson (json);
            MessagesPool.MessageSendPool.Add (message.messageId, message);
            if (onSendMessageResultEvent != null) {
                onSendMessageResultEvent (message);
            }
        }
